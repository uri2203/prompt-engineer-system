...
# --- INTERFAZ CORPORATE TECH TOTALMENTE RESTAURADA ---
    ...
    <main class="flex-1 flex p-8 gap-6 bg-[#0B1120] overflow-hidden">
        <div class="w-[55%] flex flex-col gap-6 overflow-y-auto pr-2">
            ...
            <div id="ui_mod_0" class="module-content block">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="label-red block mb-1.5">SELECCIONAR PROYECTO</label>
                    ...
                    <button onclick="guardarADN()" class="w-full bg-[#10b981] hover:bg-emerald-500 py-3 rounded-lg font-bold text-xs transition-all">ACTUALIZAR ADN EN JSON</button>
                </div>
            </div>

            <div id="ui_mod_2" class="module-content hidden">
                <h2 class="text-2xl font-bold mb-6 text-white">Motor de Guiones (Retención)</h2>
                <div class="space-y-4">
                    <div>
                        <label class="label-red block mb-1.5">MARCA ACTIVA</label>
                        <select id="m2_marca" class="w-full p-2.5 text-sm">
                            <option>La Viuda</option><option>Monkygraff</option><option>TuIALista</option>
                            <option>Ezzenshop</option><option>Yayika Digital</option><option>Yayika Apparel</option>
                        </select>
                    </div>
                    <div><label class="label-blue block mb-1.5">TEMA BASE</label><textarea id="m2_tema" class="w-full h-40 p-2.5 text-sm"></textarea></div>
                </div>
            </div>

            <div id="ui_mod_3" class="module-content hidden">
                <h2 class="text-2xl font-bold mb-6 text-white">Micro-Hooks (Continuidad)</h2>
                <div class="space-y-4">
                    <div><label class="label-green block mb-1.5">RACCORD (BLOQUE ANTERIOR)</label><textarea id="m3_raccord" class="w-full h-24 p-2.5 text-sm"></textarea></div>
            ...
            <div id="ui_mod_5" class="module-content hidden">
                ...
                <div class="space-y-5">
                    ...
                    <div><label class="label-green block mb-1.5">RACCORD VISUAL ANTERIOR</label><input type="text" id="m5_raccord" class="w-full p-2.5 text-sm"></div>
                </div>
            </div>

            <button onclick="ejecutar()" id="btn_main" class="w-full bg-[#2563eb] hover:bg-blue-500 py-4 mt-2 rounded-lg font-bold text-xs tracking-widest transition-all">COMPILAR Y EJECUTAR PROMPT</button>
        </div>
    ...
    <script>
        ...
        async function ejecutar() {
            ...
            if(moduloActivo === 'mod_2') {
                payload.datos = { marca: document.getElementById('m2_marca').value, tema: document.getElementById('m2_tema').value };
            } else if (moduloActivo === 'mod_5') {
                payload.datos = { gatillo: document.getElementById('m5_gatillo').value, duracion: document.getElementById('m5_duracion').value, avatar: document.getElementById('m5_avatar').value, raccord: document.getElementById('m5_raccord').value };
            }
...
def login():
    ...
    return render_template_string("LOGIN_HTML_HERE") # Usar el login previo
...
def ejecutar_prompt():
    ...
    if mod_id == 'mod_2':
        ...
        prompt = f"Actúa para {d['marca']}. Reglas: {info.get('reglas')}. Tema: {d['tema']}. NO SALUDES."
    elif mod_id == 'mod_5':
        prompt = f"Genera secuencia UGC. Gatillo: {d.get('gatillo')}. Duración: {d.get('duracion')}. Avatar: {d.get('avatar')}. Raccord: {d.get('raccord')}"
    ...
    return jsonify({'error': 'Falla de cuota.'}), 500
